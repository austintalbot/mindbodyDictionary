...
namespace MindBodyDictionaryMobile.PageModels
{
	...
	public partial class ConditionSummaryPageModel : ObservableObject, IQueryAttributable
	{
		...
		public ConditionSummaryPageModel(ConditionRepository conditionRepository, ModalErrorHandler errorHandler, ILogger<ConditionSummaryPageModel> logger) // Modify constructor
		{
		...
		private async Task LoadConditionSummary(string id, string type)
		{
			try
			{
				...
				if (type == "Negative")
				{
					...
					CachedImageSource = condition.CachedImageOneSource; // Assuming this property exists on MbdCondition
				}
				else if (type == "Positive")
				{
					MindsetText = "Healing Mindset"; // Example text
					Summary = condition.SummaryPositive;
					CachedImageSource = condition.CachedImageTwoSource; // Assuming this property exists on MbdCondition
				}
				else
				{
